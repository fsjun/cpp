...
void Account::regcCb(struct pjsip_regc_cbparam* param)
{
    if (param->status != PJ_SUCCESS) {
        ...
        INFOLN("SIP registration failed, status={} ({:.{}})", param->code, param->reason.ptr, param->reason.slen);
        pjsip_regc_destroy(mRegc);
        mRegc = nullptr;
        /* Stop keep-alive timer if any. */
        update_keep_alive(PJ_FALSE, nullptr);
        ...
        if (param->expiration < 1) {
            ...
            /* Stop keep-alive timer if any. */
            update_keep_alive(PJ_FALSE, nullptr);
            INFOLN("{}: unregistration success", mUser);
        } else {
            INFOLN("{}: registration success, status={} ({:.{}}), will re-register in {} seconds", mUri, param->code, param->reason.ptr, param->reason.slen, param->expiration);
            /* Start keep-alive timer if necessary. */
            update_keep_alive(PJ_TRUE, param);
        }
    } else {
        INFOLN("SIP registration updated status={}", param->code);
    }
    auto cb = mCb;
    if (cb) {
        string reason;
...
void Account::update_keep_alive(pj_bool_t start, struct pjsip_regc_cbparam* param)
{
    ...
    if (start) {
        ...
        INFOLN("Keep-alive timer started for acc {}, destination:{}, interval:{}s", mUri, addr, mKaInterval);
    }
}
...
void Account::keepAliveTimerCb(pj_timer_heap_t* th, pj_timer_entry* te)
{
    ...
    DEBUGLN("Sending {} bytes keep-alive packet for acc {} to {}", mKaData.size(), mUser, pj_sockaddr_print(&mKaTarget, addrtxt, sizeof(addrtxt), 3));
