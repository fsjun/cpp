...
int Redis::connect()
{
    ...
    if (mContext == nullptr) {
        ERRLN("redis Connection error: can't allocate redis context");
        return -1;
    }
    if (mContext->err) {
        ERRLN("redis Connection error: {}", mContext->errstr);
        redisFree(mContext);
        mContext = nullptr;
        return -1;
    }
    if (!mPassword.empty()) {
        int ret = auth();
        if (ret < 0) {
            ERRLN("auth error, password:{}", mPassword);
            redisFree(mContext);
            mContext = nullptr;
            return -1;
        }
...
int Redis::auth()
{
    ...
    if (!reply) {
        if (mContext->err == REDIS_ERR_EOF || mContext->err == REDIS_ERR_IO) {
            ERRLN("auth error:{}", mContext->errstr);
            disconnect();
            return -1;
        } else {
            ERRLN("auth error:{}", mContext->errstr);
    ...
    if (reply->type == REDIS_REPLY_ERROR) {
        ERRLN("error:{:.{}}", reply->str, reply->len);
...
Json::Value Redis::getValue(struct redisReply* reply)
{
    ...
    if (reply->type == REDIS_REPLY_STRING) {
        ...
        if (size % 2 == 0) {
            for (i = 0; i < size / 2; ++i) {
                ...
                if (elementField->type == REDIS_REPLY_STRING) {
                    ...
                    ERRLN("execute the value type[{}] is error", elementField->type);
                }
            }
            return root;
        }
        ...
        ERRLN("execute the value type[{}] is error", reply->type);
    }
    return root;
}
...
int Redis::execute(const char* format, va_list ap)
{
    ...
    if (!reply) {
        if (mContext->err == REDIS_ERR_EOF || mContext->err == REDIS_ERR_IO) {
            ERRLN("execute error:{}", mContext->errstr);
            disconnect();
            return -1;
        } else {
            ERRLN("del error:{}", mContext->errstr);
            disconnect();
            return -1;
        }
    }
    ...
    if (reply->elements > 0) {
        ...
        while (i < reply->elements - 1) {
            ...
            if (elementValue->type == REDIS_REPLY_STRING) {
                ...
                ERRLN("execute the value type[{}] is error", elementValue->type);
            }
            i += 2;
        }
    }
