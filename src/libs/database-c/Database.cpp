...
int Database::connect()
{
	if(mConnection == NULL)
	{
		...
		{			
			ERR("Database::mysql real connect error:%s\n",mysql_error(mConnection));
...
int Database::execSql(string sql)
{
	...
	if (result != 0) 
	{
		ERR("sql[%s], error code:%d:%s\n",sql.c_str(),result, mysql_error(mConnection));
		return -1;	
	}
...
int Database::execSql(string sql, vector< map<string,string> > &result)
{
	...
	if(errorCode != 0)
	{
		ERR("sql[%s], error code:%d:%s\n",sql.c_str(),errorCode, mysql_error(mConnection));
		return -1;
	}
	...
	if(result_res == NULL)
	{
		ERR("sql[%s], error:%s\n",sql.c_str(), mysql_error(mConnection));
		return -1;
	}
