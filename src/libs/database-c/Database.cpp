...
int Database::connect()
{
    if (mConnection == NULL) {
        ...
        if (mysql_real_connect(mConnection, mIp.c_str(), mName.c_str(), mPasswd.c_str(), mDb.c_str(), mPort, NULL, 0) == NULL) {
            ERRLN("Database::mysql real connect error:{}", mysql_error(mConnection));
...
int Database::execSql(string sql)
{
    ...
    if (result != 0) {
        ERRLN("sql[{}], error code:{}:{}", sql, result, mysql_error(mConnection));
        return -1;
    }
...
int Database::execSql(string sql, vector<map<string, string>>& result)
{
    ...
    if (errorCode != 0) {
        ERRLN("sql[{}], error code:{}:{}", sql, errorCode, mysql_error(mConnection));
        return -1;
    }
    ...
    if (result_res == NULL) {
        ERRLN("sql[{}], error:{}", sql, mysql_error(mConnection));
        return -1;
    }
