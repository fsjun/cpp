...
HttpClient::HttpClient() {
	...
	if (ret != CURLE_OK)
	{
		ERR("curl_global_init() failed[%d]: %s\n", ret, curl_easy_strerror(ret));
	    return;
	}
...
int HttpClient::request(string url, int post, long timeout, map<string, string> *headers, map<string,any> *param, int& code, string& contentType, string& content) {
	...
	if (res != CURLE_OK) {
		ERR("curl_easy_perform() failed[%d]: %s\n", ret, curl_easy_strerror(res));
		ret = -1;
		goto end;
	}
	...
	content = oss.str();
	INFO("http post[%d] url[%s] timeout[%d] form[%s] code[%d] contentType[%s] content[%s]\n", post, url.c_str(), timeout, p.c_str(), code, contentType.c_str(), content.c_str());
end:
	if (headerlist) {
