...
int AmqpServer::sendInit()
{
	...
	if(rpc_reply.reply_type != AMQP_RESPONSE_NORMAL)
	{
		ERR("declaring exchange error:%d\n",rpc_reply.reply_type);
		return -1;
	}
...
int AmqpServer::receiveInit()
{
	...
	if(rpc_reply.reply_type !=  AMQP_RESPONSE_NORMAL)
	{
		ERR("consuming error:%d\n",rpc_reply.reply_type);
		return -1;
	}

	DEBUG("the queuename is %.*s\n", queuenameByte.len,queuenameByte.bytes);
...
int AmqpServer::receiveFunc()
{
	while(true)
	{
		...
		{
			ERR("the res.reply_type = %d,res.library_error=%d\n",res.reply_type,res.library_error);
        ...
        DEBUG("recv from amqp message is %.*s\n", (int)envelope.message.body.len, (char *)envelope.message.body.bytes);
        int ret = 0;
...
int AmqpServer::send(string routingKey, map<string, any> &root)
{
	...
	if(root.empty())
	{
		ERR( "the rootInfo is empty for routingKey:%s\n", routingKey.c_str());
		return -1;
	}
	int ret = mJsoncpp.jsonToString(root, sendString);
	if(sendString.empty())
	{
		ERR("jsoncpp gen error ret[%d]", ret);
		return -1;
	}
	...
	if (ret < 0)
	{
		ERR("publishing msg error:%s\n",routingKey.c_str());
		return -1;
	}
	INFO("send ok exchange[%s] routingKey=[%s] msg=[%s]\n", mExchangeName.c_str(), routingKey.c_str(),sendString.c_str());
	return 0;
}
