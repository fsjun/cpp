...
void HttpClientSession::httpPost(string host, int port, string target, string query, string contentType, string body)
{
    ...
    mUrl = boost::str(boost::format("http://%s:%d%s%s") % host % port % target % query);
    INFOLN("http post, url:{} contentType:{} body:{}", mUrl, contentType, body);
    // Look up the domain name
    mResolver.async_resolve(host, std::to_string(port), beast::bind_front_handler(&HttpClientSession::on_resolve, shared_from_this()));
}
...
{
    if (ec) {
        ERRLN("http resolve error, url:{} msg:{}", mUrl, ec.message());
...
{
    if (ec) {
        ERRLN("http connect error, url:{} msg:{}", mUrl, ec.message());
...
void HttpClientSession::on_write(beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    if (ec) {
        ERRLN("http write error, url:{} local {}:{} msg:{}", mUrl, mLocalIp, mLocalPort, ec.message());
        mCb(false, 500, "write", "");
        return;
    }
...
void HttpClientSession::on_read(beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    if (ec) {
        ...
        ERRLN("http read error, url:{} local {}:{} msg:{}", mUrl, mLocalIp, mLocalPort, ec.message());
        mCb(false, 500, "read", "");
        return;
    }
    ...
    INFOLN("http success, url:{} code:{} reason:{} body:{} local {}:{}", mUrl, code, reason, body, mLocalIp, mLocalPort);
    mCb(true, code, reason, body);
    // Gracefully close the socket
    mStream.socket().shutdown(tcp::socket::shutdown_both, ec);
    // not_connected happens sometimes so don't bother reporting it.
    if (ec && ec != beast::errc::not_connected) {
        ERRLN("http shutdown error, url:{} local {}:{} msg:{}", mUrl, mLocalIp, mLocalPort, ec.message());
        return;
    }
