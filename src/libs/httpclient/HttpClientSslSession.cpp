...
void HttpClientSslSession::httpPost(string host, int port, string target, string query, string contentType, string body)
{
    ...
    mUrl = boost::str(boost::format("https://%s:%d%s%s") % host % port % target % query);
    INFOLN("https post, url:{} contentType:{} body:{}\n", mUrl, contentType, body);
    // Set SNI Hostname (many hosts need this to handshake successfully)
    if (!SSL_set_tlsext_host_name(mStream.native_handle(), host.c_str())) {
        beast::error_code ec { static_cast<int>(::ERR_get_error()), net::error::get_ssl_category() };
        ERRLN("set tlsext host name error, msg:{}\n", ec.message());
...
{
    if (ec) {
        ERRLN("https resolve error, url:{} msg:{}\n", mUrl, ec.message());
...
{
    if (ec) {
        ERRLN("https connect error, url:{} msg:{}\n", mUrl, ec.message());
...
{
    if (ec) {
        ERRLN("https handshake error, url:{} local {}:{} msg:{}\n", mUrl, mLocalIp, mLocalPort, ec.message());
...
void HttpClientSslSession::on_write(beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    if (ec) {
        ERRLN("https write error, url:{} local {}:{} msg:{}\n", mUrl, mLocalIp, mLocalPort, ec.message());
        mCb(false, 500, "write", "");
        return;
    }
...
void HttpClientSslSession::on_read(beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    if (ec) {
        ...
        ERRLN("https read error, url:{} local {}:{} msg:{}\n", mUrl, mLocalIp, mLocalPort, ec.message());
        mCb(false, 500, "read", "");
        return;
    }
    ...
    INFOLN("https get success, url:{} code:{} reason:{} body:{} local {}:{}\n", mUrl, code, reason, body, mLocalIp, mLocalPort);
...
void HttpClientSslSession::on_shutdown(beast::error_code ec)
{
    ...
    if (ec) {
        ERRLN("https shutdown error, url:{} local {}:{} msg:{}\n", mUrl, mLocalIp, mLocalPort, ec.message());
        return;
    }
