...
int Encoding::Convert(string from_code, string to_code, string src, string& dst)
{
    ...
    if (nullptr == h_iconv) {
        ERRLN("iconv do not support from {} to {}", from_code, to_code);
        return -1;
    }
...
int Encoding::Utf8ToSystemEncoding(string src, string& dst)
{
    ...
    try {
        ...
        ERRLN("utf8 to system encoding error, exception: {}", boost::diagnostic_information(e));
    } catch (const std::exception& e) {
        ERRLN("utf8 to system encoding error, exception: {}", e.what());
    } catch (...) {
