...
{
    if (ec) {
        ERRLN("websocket accept error, remote {}:{} msg:{}", mRemoteIp, mRemotePort, ec.message());
        return;
    }
...
void WsServerSession::on_read(beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    if (ec == websocket::error::closed) {
        INFOLN("ws close by peer, remote {}:{}", mRemoteIp, mRemotePort);
        mCb(shared_from_this(), WS_SERVER_CLOSE, nullptr, 0);
        return;
    }
    if (ec) {
        ERRLN("ws read error, remote {}:{} msg:{}", mRemoteIp, mRemotePort, ec.message());
...
{
    INFOLN("ws send message, remote {}:{} message:{}", mRemoteIp, mRemotePort, message);
...
{
    DEBUGLN("ws send data, remote {}:{}", mRemoteIp, mRemotePort);
    std::unique_lock<std::mutex> l(mMutex);
    if (async) {
        if (mIsSendError) {
            return -1;
...
void WsServerSession::on_write(shared_ptr<std::vector<char>> vec, beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    boost::ignore_unused(bytes_transferred);
    if (ec) {
        ERRLN("ws write error, remote {}:{} msg:{}", mRemoteIp, mRemotePort, ec.message());
        mIsSendError = true;
        mQueue.clear();
        return;
    }
