...
{
    INFOLN("wss write, remote {}:{} local {}:{} message:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, message);
    mStream.text(true);
    if (async) {
        // Send the message
        ...
        if (ec) {
            ERRLN("error ws write, remote {}:{} local {}:{}, msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
            return;
        }
    }
}
...
{
    DEBUGLN("ws write, remote {}:{} local {}:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort);
    mStream.binary(true);
    if (async) {
        // Send the message
        ...
        if (ec) {
            ERRLN("error ws write, remote{}s:{} local {}:{}, msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
            return;
        }
    }
}
...
void WsClientSslSession::on_write(beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    if (ec) {
        ERRLN("error wss write, remote {}:{} local {}:{}, msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
        return;
    }
}
...
{
    if (ec) {
        ERRLN("error wss resolve, remote {}:{}, msg:{}", mHost, mRemotePort, mLocalIp, mLocalPort, ec.message());
...
{
    if (ec) {
        ERRLN("error ws connect, remote {}:{}, msg:{}", mRemoteIp, mRemotePort, ec.message());
        mCb(shared_from_this(), WS_CLIENT_ERROR, nullptr, 0);
        return;
    }
...
{
    if (ec) {
        ERRLN("error wss ssl handshake, remote {}:{} local {}:{} msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
        mCb(shared_from_this(), WS_CLIENT_ERROR, nullptr, 0);
        return;
    }
...
{
    if (ec) {
        ERRLN("error wss handshake, remote {}:{} local {}:{} msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
        mCb(shared_from_this(), WS_CLIENT_ERROR, nullptr, 0);
        return;
    }
...
void WsClientSslSession::on_read(beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    if (ec == websocket::error::closed) {
        INFOLN("close wss, remote {}:{} local {}:{} msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
        mCb(shared_from_this(), WS_CLIENT_CLOSE, nullptr, 0);
        return;
    }
    if (ec) {
        ERRLN("error wss read, remote {}:{} local {}:{} msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
        mCb(shared_from_this(), WS_CLIENT_ERROR, nullptr, 0);
        return;
    }
    ...
    if (isBinary) {
        ...
        DEBUGLN("wss receive message, remote {}:{}", mRemoteIp, mRemotePort);
    } else {
        INFOLN("wss receive message, remote {}:{} message:{:.{}}", mRemoteIp, mRemotePort, (char*)data.data(), data.size());
    }
...
{
    if (ec) {
        ERRLN("error ws close, remote {}:{} local {}:{} msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
        return;
    }
}
