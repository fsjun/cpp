...
WebSocketClient::WebSocketClient(shared_ptr<boost::asio::io_context> ioContext)
{
    mIoContext = ioContext;
}

int WebSocketClient::connect(string url_s, WebSocketClientCb cb)
{
    Url url;
    url.parse(url_s);
    if ("ws" == url.protocol) {
        auto session = make_shared<WsClientSession>(mIoContext, url.host, url.port, url.path, url.query, cb);
        session->start();
    } else if ("wss" == url.protocol) {
        ...
        ERR("invalid ws url, url:%s", url_s.c_str());
    }
    return 0;
}

string WebSocketClient::GetUserAgent()
{
