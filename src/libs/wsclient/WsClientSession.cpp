...
{
    INFOLN("ws write, remote {}:{} local {}:{} message:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, message);
    mStream.text(true);
    if (async) {
        // Send the message
...
{
    if (ec) {
        ERRLN("error ws resolve, remote {}:{}, msg:{}", mHost, mRemotePort, mLocalIp, mLocalPort, ec.message());
...
{
    if (ec) {
        ERRLN("error ws handshake, remote {}:{} local {}:{} msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
        mCb(shared_from_this(), WS_CLIENT_ERROR, nullptr, 0);
        return;
    }
...
void WsClientSession::on_read(beast::error_code ec, std::size_t bytes_transferred)
{
    ...
    if (ec == websocket::error::closed) {
        INFOLN("close ws, remote {}:{} local {}:{} msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
        mCb(shared_from_this(), WS_CLIENT_CLOSE, nullptr, 0);
        return;
    }
    if (ec) {
        ERRLN("error ws read, remote {}:{} local {}:{} msg:{}", mRemoteIp, mRemotePort, mLocalIp, mLocalPort, ec.message());
        mCb(shared_from_this(), WS_CLIENT_ERROR, nullptr, 0);
        return;
    }
    ...
    if (isBinary) {
        ...
        DEBUGLN("ws receive message, remote {}:{}", mRemoteIp, mRemotePort);
    } else {
        INFOLN("ws receive message, remote {}:{} message:{:.{}}", mRemoteIp, mRemotePort, (char*)data.data(), data.size());
    }
